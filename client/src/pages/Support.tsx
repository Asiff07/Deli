    ...
    <div className="w-full bg-[#050505] min-h-screen text-white pt-24 pb-16 px-6 md:px-16 font-sans">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-12">
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-8 text-left">
          <div>
            <span className="text-[10px] text-neon-cyan tracking-widest font-semibold uppercase">Frequently Queried Logs</span>
            <h1 className="font-display font-bold text-3xl md:text-5xl mt-1 text-white">TECHNICAL FAQ</h1>
          </div>

          <div className="flex flex-col gap-6 mt-2">
            {faqs.map((faq, idx) => (
              <div key={idx} className="glass-panel p-5 rounded-xl border border-white/5 flex flex-col gap-2">
                <h3 className="font-display font-bold text-xs text-neon-cyan flex items-center gap-2">
                  <HelpCircle size={14} className="shrink-0" />
                  {faq.q}
                </h3>
                <p className="text-[11px] text-white/50 leading-relaxed pl-5 font-sans">
                  {faq.a}
                </p>
              </div>
            ))}
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-6">
          <div className="glass-panel p-6 md:p-8 rounded-2xl border border-white/15 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-neon-cyan/5 rounded-full filter blur-[60px]" />
            
            <div className="flex flex-col gap-1 mb-6">
              <span className="text-[10px] text-neon-cyan tracking-widest font-semibold uppercase">OPERATOR HELPDESK</span>
              <h2 className="font-display font-bold text-xl text-white">OPEN SUPPORT TICKET</h2>
            </div>

            {error && (
              <div className="p-3 bg-neon-rose/5 border border-neon-rose/25 rounded-lg flex items-start gap-2 text-xs text-neon-rose mb-5">
            ...
            {success && (
              <div className="p-3 bg-neon-cyan/5 border border-neon-cyan/20 rounded-lg flex items-start gap-2 text-xs text-neon-cyan mb-5 animate-pulse">
            ...
            <form onSubmit={handleTicketSubmit} className="flex flex-col gap-4 text-xs">
              <div className="flex flex-col gap-1">
                <label className="text-white/60">Registered Contact Email</label>
                <div className="relative">
                  <input
                    ...
                    required
                    disabled={!!user}
                    className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg pl-10 pr-4 py-3 text-white focus:outline-none focus:border-neon-cyan disabled:opacity-50"
                  />
              ...
              <div className="flex flex-col gap-1">
                <label className="text-white/60">Ticket Category</label>
                <select
                  value={category}
                  onChange={(e: any) => setCategory(e.target.value)}
                  className="bg-[#0a0a0a] border border-white/10 rounded-lg px-3.5 py-3 text-white focus:outline-none focus:border-neon-cyan cursor-pointer"
                >
              ...
              <div className="flex flex-col gap-1">
                <label className="text-white/60">Ticket Subject Summary</label>
                <div className="relative">
                  <input
                    type="text"
                    ...
                    required
                    className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg pl-10 pr-4 py-3 text-white focus:outline-none focus:border-neon-cyan"
                  />
                  <FileText className="absolute left-3.5 top-3.5 text-white/30" size={14} />
                </div>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-white/60">Detailed Message Log</label>
                <textarea
                  placeholder="Detail your request (minimum 10 characters)..."
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  required
                  className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-neon-cyan min-h-[120px]"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full liquid-glass-cyan py-3.5 rounded-lg text-xs font-semibold tracking-wide mt-2 cursor-pointer disabled:opacity-50"
              >

    ...
    <div className="w-full min-h-screen bg-[#050505] text-white pt-32 pb-24 px-6 md:px-16 flex flex-col items-center">
      <div className="max-w-4xl w-full flex flex-col gap-12 text-center items-center">
        ...
        <div className="flex flex-col gap-4 items-center">
          <div className="glass-capsule px-4 py-1 text-[10px] text-neon-cyan tracking-widest font-semibold uppercase">
            Our Heritage
          </div>
          <h1 className="font-display font-bold text-4xl md:text-6xl tracking-tight leading-none">
            Bespose Synthesis <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-neon-cyan via-neon-yellow to-neon-cyan bg-size-200 animate-pulse-slow">
              Of Luxury Assets
            </span>
          </h1>
          <p className="text-sm md:text-base text-white/50 max-w-2xl leading-relaxed mt-2">
            Lumen x Deli is a luxury technology house founded to bridge the boundary between mechanical physics, high art, and premium manufacturing.
          </p>
        </div>

        {/* Pillars Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left mt-6">
          <div className="glass-panel p-8 rounded-2xl flex flex-col gap-4 border border-white/5 relative overflow-hidden group">
            <Cpu className="text-neon-cyan" size={28} />
            <h3 className="font-display font-semibold text-lg text-white">Parametric Precision</h3>
          ...
          <div className="glass-panel p-8 rounded-2xl flex flex-col gap-4 border border-white/5 relative overflow-hidden group">
            ...
            <h3 className="font-display font-semibold text-lg text-white">Chemical Finishing</h3>
            <p className="text-xs text-white/50 leading-relaxed">
          ...
          <div className="glass-panel p-8 rounded-2xl flex flex-col gap-4 border border-white/5 relative overflow-hidden group">
            ...
            <h3 className="font-display font-semibold text-lg text-white">Sintering Labs</h3>
            <p className="text-xs text-white/50 leading-relaxed">
        ...
        <div className="glass-panel p-8 md:p-12 rounded-3xl border border-white/5 relative w-full overflow-hidden mt-6">
          ...
          <h2 className="font-display font-bold text-xl md:text-2xl text-white tracking-wide italic">
            "A fusion of structural dynamics and optical art."
          </h2>
          <p className="text-xs text-white/40 leading-relaxed max-w-2xl mx-auto mt-4">
        ...
        <div className="w-full flex flex-col gap-8 text-left mt-6">
          <div className="text-center md:text-left">
            <span className="text-[10px] text-neon-yellow tracking-widest font-semibold uppercase">THE FOUNDING TEAM</span>
            <h2 className="font-display font-bold text-2xl md:text-3xl text-white mt-1">PEOPLE BEHIND THE SINTERING</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div 
                key={idx} 
                className="glass-panel p-6 rounded-2xl flex flex-col gap-4 border border-white/5 relative overflow-hidden"
              >
                {/* Team Avatar (Fallback to initials, ready for image source url) */}
                <div className="w-16 h-16 rounded-full overflow-hidden border border-white/10 bg-white/5 flex items-center justify-center font-display font-bold text-base text-neon-cyan tracking-wider">
                  {member.imgUrl ? (
                ...
                <div>
                  <h3 className="font-display font-semibold text-sm text-white">{member.name}</h3>
                  <span className="text-[10px] text-neon-cyan/80 font-semibold tracking-wide block mt-0.5">{member.role}</span>
                </div>
        ...
        <button
          ...
          className="liquid-glass-cyan px-8 py-3.5 rounded-full text-xs font-semibold tracking-wider cursor-pointer"
        >
          Explore the Catalog
        </button>
      </div>
